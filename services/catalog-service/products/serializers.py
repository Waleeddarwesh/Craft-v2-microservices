...
class CollectionSerializer(serializers.ModelSerializer):
    ...
    def get_images(self, obj):
        first_items = list(obj.items.all())[:4]  
        image_urls = []
    ...
    def validate_items(self, value):
        ...
        if invalid_products:
            raise serializers.ValidationError(
                _("You can only add your own products. Invalid product IDs: {invalid_products}").format(invalid_products=invalid_products)
            )
        ...
        if collection:
            ...
            if duplicate_products:
                raise serializers.ValidationError(
                    _("These products have already been added to the collection: {duplicate_products}").format(duplicate_products=list(duplicate_products))
                )
...
class LatestCollectionSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, obj):
        ...
        if first_item and first_item.product:
            images = list(first_item.product.images.all())
            if images:
                first_image = images[0]
