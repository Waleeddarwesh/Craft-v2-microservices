...
const Sidebar = (() => {
    const ICONS = {
        ...
        audit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>',
    ...
    const NAV_ITEMS = [
        {
            section: window.t('Analytics'), items: [
                { route: 'audit-logs', label: window.t('Audit Logs'), icon: 'audit' },
            ]
        },
        {
            section: window.t('Infrastructure'), items: [
                { route: 'servers', label: window.t('Servers'), icon: 'overview' },
                { route: 'services', label: window.t('Services'), icon: 'settings' },
                { route: 'storage', label: window.t('Storage'), icon: 'overview' },
                { route: 'containers', label: window.t('Containers'), icon: 'products' },
            ]
        },
        {
        ...
        },
        {
            section: window.t('Operations'), items: [
                { route: 'system-logs', label: window.t('System Logs'), icon: 'reports' },
                { route: 'backups', label: window.t('Backups'), icon: 'audit' },
                { route: 'cron-jobs', label: window.t('Cron Jobs'), icon: 'settings' },
                { route: 'incidents', label: window.t('Incidents'), icon: 'notifications' },
            ]
        },
        {
            section: window.t('Automation'), items: [
                { route: 'config-management', label: window.t('Config Management'), icon: 'settings' },
                { route: 'file-explorer', label: window.t('File Explorer'), icon: 'overview' },
                { route: 'automation', label: window.t('Scripts'), icon: 'audit' },
            ]
        },
        {
    ...
    function render(container) {
        ...
        let html = `
            <div class="sidebar-brand" onclick="Router.navigate('#servers')" style="cursor: pointer;">
                <img src="/sysadmin/logo.jpg" alt="Craft" class="sidebar-brand-logo">
                <span class="sidebar-brand-text">${window.t('Craft SysAdmin')}</span>
            </div>
            <nav class="sidebar-nav">`;

        const authorizedModules = window.UserIdentity ? window.UserIdentity.modules.map(m => m.key) : [];

        NAV_ITEMS.forEach(section => {
            const visibleItems = section.items.filter(item => authorizedModules.includes(item.route));
        ...
        html += `</nav>
            <div class="sidebar-footer">
                <div class="nav-item" onclick="Auth.logout()">
                    ${ICONS.logout}
                    <span class="nav-label">${window.t('Sign Out')}</span>
                </div>
            </div>`;
